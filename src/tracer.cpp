...
std::vector<std::string> Tracer::loggers_;
...
void Tracer::init(const std::string& name, const std::string& path,
  ...
  loggers_.emplace_back(name);
}

void Tracer::set_level(const std::string& level) {
  for (auto logger : loggers_) {
    set_level(level, logger);
  }
}

void Tracer::set_level(const std::string& level, const std::string& name) {
...
int Tracer::get_level() { return get_level(DEFAULT_LOGGER_NAME); }

int Tracer::get_level(const std::string& name) {
