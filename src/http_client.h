...
class HttpClient {
 public:
  ...
  std::string post_json_with_auth(const std::string& url,
                                  const std::string& json_body,
                                  const std::string& token);

  // POST request with JSON body, authorization header, and additional headers
  std::string post_json_with_auth(
  ...
  std::string delete_with_auth(const std::string& url,
                               const std::string& token);
 ...
 private:
  static size_t write_callback(void* contents, size_t size, size_t nmemb,
                               void* userp);
  std::string perform_request(const std::string& url, void* headers,
                              const char* method = nullptr,
                              const char* post_data = nullptr);
  std::string perform_request_internal(const std::string& url, void* headers,
                                       const char* method,
