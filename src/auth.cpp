...
AuthManager::AuthManager(const std::string& chain)
    : impl_(new Impl()),
      chain_(chain),
      auth_base_url_("https://api.standx.com") {
  if (sodium_init() < 0) {
...
AuthManager::~AuthManager() { delete impl_; }
...
void AuthManager::set_private_key(const std::string& private_key_hex) {
  ...
  if (!secp256k1_ec_pubkey_create(impl_->ctx, &pubkey,
                                   private_key_bytes_.data())) {
    throw std::runtime_error("failed to create public key from private key");
  }

  unsigned char pubser[65];
  size_t pubserlen = 65;
  secp256k1_ec_pubkey_serialize(impl_->ctx, pubser, &pubserlen, &pubkey,
                                 SECP256K1_EC_UNCOMPRESSED);
  address_ = derive_eth_address(pubser);
}

std::string AuthManager::get_address() const { return address_; }
...
std::string AuthManager::login(int expires_seconds) {
  ...
  std::string request_id =
      base58_encode(impl_->ed25519_pk, crypto_sign_PUBLICKEYBYTES);

  HttpClient http;
  std::string prepare_url =
      auth_base_url_ + "/v1/offchain/prepare-signin?chain=" + chain_;
  json jreq;
  ...
  std::string login_url =
      auth_base_url_ + "/v1/offchain/login?chain=" + chain_;
  json jlogin;
  ...
  std::string access_token =
      jlogin_resp.value("accessToken", jlogin_resp.value("token", ""));
...
std::string AuthManager::sign_message(const std::string& message) {
  ...
  keccak256(reinterpret_cast<const unsigned char*>(prefixed.data()),
            prefixed.size(), msghash);

  secp256k1_ecdsa_recoverable_signature sig;
  if (!secp256k1_ecdsa_sign_recoverable(impl_->ctx, &sig, msghash,
                                         private_key_bytes_.data(), nullptr,
                                         nullptr)) {
  ...
  int recid = 0;
  secp256k1_ecdsa_recoverable_signature_serialize_compact(impl_->ctx, sig64,
                                                          &recid, &sig);

  unsigned char outsig[65];
  memcpy(outsig, sig64, 64);
  outsig[64] = static_cast<unsigned char>(recid + 27);
...
std::string AuthManager::sign_ed25519_base64(const std::string& message) {
  ...
  crypto_sign_detached(signature, &sig_len,
                       reinterpret_cast<const unsigned char*>(message.data()),
                       message.size(), impl_->ed25519_sk);
