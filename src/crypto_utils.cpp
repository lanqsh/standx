...
static void keccak_256_impl(const unsigned char *in, size_t inlen, unsigned char *out) {
    u64 s[25];
...
std::string eip55_checksum_address(const unsigned char* addr_bytes, size_t len) {
    ...
    keccak_256_impl((const unsigned char*)addr_hex.data(), addr_hex.size(), addr_hash);
