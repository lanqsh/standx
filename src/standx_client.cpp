...
std::string StandXClient::query_order(int order_id,
...
std::string StandXClient::new_order(const std::string& symbol,
  ...
  if (symbol.empty() || side.empty() || order_type.empty() || qty.empty() ||
      time_in_force.empty()) {
    throw std::runtime_error(
        "symbol, side, order_type, qty, and time_in_force are required");
  }
...
std::string StandXClient::cancel_order(int order_id,
                                       const std::string& cl_ord_id) {
  if (access_token_.empty()) {
    throw std::runtime_error("not logged in, call login() first");
  }
